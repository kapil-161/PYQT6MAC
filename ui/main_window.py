...
class MainWindow(QMainWindow):
    ...
    def load_experiments(self):
        try:
            if not self.selected_folder:
                logging.warning("No folder selected, skipping experiment load")
                return
            logging.info(f"Loading experiments for folder: {self.selected_folder}")
            ...
            for exp_name, filename in experiments:
                logging.info(f"Adding experiment to selector: {exp_name} ({filename})")
                self.experiment_selector.addItem(exp_name, userData=filename)
            if experiments:
                self.experiment_selector.setCurrentIndex(0)
                logging.info(f"Set current experiment to index 0")
            else:
                logging.warning("No experiments found for the selected folder")
        except Exception as e:
            logging.error(f"Error loading experiments: {e}", exc_info=True)
    ...
    def on_folder_changed(self):
        ...
        logging.info(f"Folder changed to: {self.selected_folder}")
